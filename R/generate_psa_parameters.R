...
generate_psa_params <- function(n_sim = n_sim, seed = seed, n_pop = n_pop, scenario = scenario,
  ...
  df_psa_params <- data.frame(
    ...
    p_fent_exp_2020 = rbeta(n_sim, shape1 = df_fentanyl["shape1", "pe"], shape2 = df_fentanyl["shape2", "pe"]),### R&R MODIFICATION ###
    
    # Naloxone (OD reversal)
    
    ### R&R MODIFICATION ###
    #p_witness = rbeta(n_sim, shape1 = df_overdose["shape1", "witness_prob"], shape2 = df_overdose["shape2", "witness_prob"]), (now part of calibration)
    ...
    ### R&R MODIFICATION ###
    p_NX_2020 = rbeta(n_sim, shape1 = df_naloxone["shape1", "pe"], sd = df_naloxone["shape2", "pe"]), #Hold NX at 2020 levels
